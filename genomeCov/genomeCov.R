...
collapsed <- read_tsv("data/no_trim/ds_collapsed.tab.gz")
masked <- read_tsv("data/no_trim/ds_merged_rep_metric.tab")
flagstat <- read_tsv("data/no_trim/flagstat.tab")
ss_flagstat <- read_tsv("data/no_trim/subsampled_flagstat.tab")

trim_collapsed <- read_tsv("data/trim/ds_collapsed.tab.gz")
trim_masked <- read_tsv("data/trim/metrics.tab")
trim_flagstat <- read_tsv("data/trim/flagstat.tab")
trim_ss_flagstat <- read_tsv("data/trim/ds_flagstat.tab")
...
trim_flagstat <- trim_flagstat %>%
  gather('flagstat', 'count', -density, -ext, -swga, -rep)
trim_flagstat$flagstat <- factor(trim_flagstat$flagstat, levels = c('total', 'mapped', 'paired'))

trim_mpt <- ggplot(data = trim_flagstat, aes(x = interaction(swga, ext), y = count, fill=flagstat)) +
  geom_bar(stat='identity', position='dodge') +
  coord_polar() +
  facet_wrap(~density) +
  scale_fill_brewer(palette = "Reds") +
...
trim_ss_flagstat <- trim_ss_flagstat %>%
  gather('flagstat', 'count', -density, -ext, -swga, -rep)
trim_ss_flagstat$flagstat <- factor(trim_ss_flagstat$flagstat, levels = c('total', 'mapped', 'paired'))

trim_ss_mpt <- ggplot(data = trim_ss_flagstat, aes(x = interaction(swga, ext), y = count, fill=flagstat)) +
  geom_bar(stat='identity', position='dodge') +
  coord_polar() +
  facet_wrap(~density) +
  scale_fill_brewer(palette = "Blues") +
...
trim_wide_collapsed <- trim_collapsed %>%
  spread(key = rep, val = depth)

trim_correlation_plot <- ggplot(trim_wide_collapsed, aes(x = log10(`1`), y=log10(`2`))) +
  geom_point(shape = 21, size = 0.5, alpha = 0.5, aes(fill = interaction(swga, ext))) +
  facet_grid(ext~swga~density) +
  stat_cor() +
  theme_classic() +
...
trim_depth_percentage_plot <- ggplot(trim_depthPCT, aes(x = depth, y = percentage)) +
  geom_point(
    shape = 21, aes(
      fill = interaction(swga, ext)
      )) +
...
ggsave("plots/trim_MappedPairedTotal.pdf", trim_mpt, width = 10, height = 8)
ggsave("plots/trim_SSMappedPairedTotal.pdf", trim_ss_mpt, width = 10, height = 8)
ggsave("plots/trim_correlation_plot.png", trim_correlation_plot, width = 10, height = 8)
ggsave("plots/trim_percentile_plot.pdf", trim_depth_percentage_plot, width = 10, height = 6)
